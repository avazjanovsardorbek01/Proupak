(function(window) {
    ...
    BX.frameCache.insertBlock = function(e, a) {
        ...
        function h() {
            if (BX.Type.isStringFilled(c.html)) {
                document.head.insertAdjacentHTML("beforeend", c.html)
            }
        ...
        function u() {
            ...
            if (!BX.type.isArray(e.PROPS.STRINGS) || e.PROPS.STRINGS.length < 1) {
                return a
            }
    ...
    BX.frameCache.handleResponse = function(e) {
        ...
        if (e.isManifestUpdated == "1" && this.vars.CACHE_MODE === "APPCACHE" && window.applicationCache && (window.applicationCache.status == window.applicationCache.IDLE || window.applicationCache.status == window.applicationCache.UPDATEREADY)) {
            window.applicationCache.update()
        }
        if (e.htmlCacheChanged === true && this.vars.CACHE_MODE === "HTMLCACHE") {
    ...
    BX.frameCache.requestData = function() {
        ...
        r += (r.indexOf("?") >= 0 ? "&" : "?") + "bxrand=" + (new Date).getTime();
        BX.ajax({
            timeout: 60,
            method: "GET",
            url: r,
    ...
    BX.frameCache.insertFromCache = function(e, a) {
        if (!this.frameDataReceived) {
            ...
            BX.onCustomEvent(this, "onCacheInvokeAfter", [this.vars.storageBlocks, e])
        }
    };
    BX.frameCache.insertBlocks = function(e, a) {
        ...
        const n = () => {
            if (window.performance) {
                ...
                for (var r = 0; r < t.length; r++) {
                    var s = t[r];
                    if (s.initiatorType === "xmlhttprequest" && s.name && s.name.match(/bxrand=[0-9]+/)) {
                        this.requestTiming = s
                    }
                }
                if (window.performance.measure) {
                    window.performance.measure("Composite:LCP");
                    var i = performance.getEntriesByName("Composite:LCP");
                    if (i.length > 0 && i[0].duration) {
                        this.lcp = Math.ceil(i[0].duration)
                    }
                }
            }
    ...
    BX.frameCache.writeCache = function(e) {
        for (var a = 0; a < e.length; a++) {
            if (e[a].PROPS.USE_BROWSER_STORAGE === true) {
                this.writeCacheWithID(e[a].ID, e[a].CONTENT, e[a].HASH, JSON.stringify(e[a].PROPS))
            }
        }
    };
    ...
    BX.frameCache.readCacheWithID = function(e, a) {
        if (BX.frameCache.openDatabase()) {
            this.cacheDataBase.composite.where("ID").anyOf(e).toArray().then(function(e) {
                a({
                    items: e
                })
            }.bind(this))
